...
(function () {
  ...
  function buildFeatures(list) {
    ...
    var names = ['harga', 'komisiPct', 'klik', 'konversi', 'convRate', 'epc', 'margin', 'komisiAvail', 'roas', 'pendapatan'];
    ...
    list.forEach(function (p) {
      ...
      var x = [num(p.harga), num(p.komisiPct), num(p.klik), num(p.konversi), convRate, epc, margin, komisiAvail, roas, pendapatan];
      names.slice(10).forEach(function (n) { x.push(catRow[n]); });
  ...
  function trainModel(list, opt) {
    ...
    function buildReport(model) {
      var tr, te;
      try {
        tr = ml.evalModel(ds.trainX.map(function (r) { return model.predictProb(r); }), ds.trainY);
        te = ml.evalModel(ds.testX.map(function (r) { return model.predictProb(r); }), ds.testY, ds.trainY.filter(function (y) { return y === 1; }).length / ds.trainY.length);
      } catch (e) {
        return { error: 'Evaluasi model gagal: ' + e.message };
      }
      return {
        ok: true, engine: model.engine, kind: model.kind, model: model,
        train: tr, test: te, scaler: ds.scaler, names: ds.names, ds: ds, seed: opt.seed == null ? 42 : opt.seed
      };
    }

    if (typeof ml.trainMLPAsync === 'function') {
      return ml.trainMLPAsync(ds.trainX, ds.trainY, opt, opt.onProgress).then(buildReport).catch(function (e) {
        return { error: 'Pelatihan gagal: ' + e.message };
      });
    }
    if (typeof ml.trainMLP === 'function') {
      try { return Promise.resolve(buildReport(ml.trainMLP(ds.trainX, ds.trainY, opt))); }
      catch (e) { return Promise.resolve({ error: 'Pelatihan gagal: ' + e.message }); }
    }
    return Promise.resolve({ error: 'Engine ML tidak tersedia' });
  }
  ...
  function formatScores(s) {
    if (!s || s.error) return '⚠� ML gagal: ' + (s && s.error);
    ...
    return '## 🧠 Skor Profitabilitas ML (' + s.report.kind + ' · ' + s.report.engine + ')\n\n' +
      '_Validasi: akurasi latih ' + s.report.train.acc + '% · akurasi uji (OOS) ' + s.report.test.acc + '% · baseline ' + s.report.test.baseline + '%_\n\n' +
      '| Produk | P(untung) | Aksi | Laba |\n|---|---|---|---:|\n' + rows;
  }

  function formatForecast(f) {
    if (!f || f.error) return '⚠� ' + (f && f.error);
  ...
  function formatStrategy(s) {
    if (!s || s.error) return '⚠� ' + (s && s.error);
    return '## 🗺� Strategi Affiliator\n\n' +
