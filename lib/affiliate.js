...
(function () {
  ...
  var STORAGE_KEY_TOMB = 'cc_aff_tombstones_v1';
  ...
  function addProduct(p) {
    ...
    var list = getProducts();
    var id = p.id != null ? p.id : nextId(list);
    // hindari tabrakan id (impor/cloud membawa id lama): bila id sudah dipakai,
    // dapatkan id baru yang dijamin unik (> semua id yang ada).
    var clash = false;
    for (var i0 = 0; i0 < list.length; i0++) if (num(list[i0].id) === num(id)) { clash = true; break; }
    if (clash) id = nextId(list);
    var rec = {
      id: id,
      updatedAt: Date.now(),
  ...
  function deleteProduct(id) {
    ...
    setProducts(list);
    recordTombstone(num(removed.id));
    saveProducts();
    return { ok: true, removed: removed, count: list.length };
  }
  function clearProducts() {
    getProducts().forEach(function (p) { recordTombstone(num(p.id)); });
    setProducts([]);
    saveProducts();
    return { ok: true, count: 0 };
  }

  /* ---------- tombstone penghapusan (propagasi hapus antar perangkat via cloud) ----------
     Saat produk dihapus, id-nya disimpan sebagai tombstone. Push cloud mengubahnya
     jadi baris { _deleted:true } sehingga perangkat lain ikut menghapus produk itu.
     Tombstone hanya ephemeral: dibersihkan setelah push berhasil. */
  function getTombstones() {
    if (!CC.affTombCache) {
      var arr = [];
      try {
        var v = localStorage.getItem(STORAGE_KEY_TOMB);
        if (v) { var j = JSON.parse(v); if (Array.isArray(j)) arr = j; }
      } catch (e) {}
      CC.affTombCache = arr;
    }
    return (CC.affTombCache || []).slice();
  }
  function saveTombstones() {
    try { localStorage.setItem(STORAGE_KEY_TOMB, JSON.stringify(getTombstones())); } catch (e) {}
    return getTombstones();
  }
  function recordTombstone(id, ts) {
    if (id == null) return;
    var t = { id: String(id), updatedAt: ts == null ? Date.now() : num(ts, Date.now()) };
    var arr = getTombstones();
    var found = false;
    arr.forEach(function (x) { if (String(x.id) === String(t.id)) { found = true; if (t.updatedAt > num(x.updatedAt, 0)) x.updatedAt = t.updatedAt; } });
    if (!found) arr.push(t);
    CC.affTombCache = arr;
    saveTombstones();
    try { if (typeof CC.onAffChange === 'function') CC.onAffChange(arr); } catch (e) {}
    return arr.length;
  }
  function clearTombstones(untilTs) {
    /* tanpa argumen → hapus semua; dengan timestamp → hapus yang updatedAt <= untilTs
       (dipakai cloud setelah push tombstone berhasil dikirim). */
    var arr = getTombstones();
    var keep = untilTs == null ? [] : arr.filter(function (t) { return num(t.updatedAt, 0) > num(untilTs); });
    CC.affTombCache = keep;
    saveTombstones();
    return keep.length;
  }
  ...
  function forecast(list, opt) {
    ...
    function trendRegress(vals) {
      ...
      var b = (sy - m * sx) / n;
      return { m: m, b: b, n: n, sx: sx, sxx: sxx };
    }
    var pend = series.map(function (s) { return s.pendapatan; });
    // Fit KONSISTEN di window w periode terakhir (relevan utk tren terkini) —
    // proyeksi & kualitas fit (R², band) memakai window yang sama, tanpa paksaan
    // "tidak boleh turun" yang membuat proyeksi bias optimistis.
    var w = Math.max(2, Math.min(3, pend.length));
    var win = pend.slice(pend.length - w);
    var tWin = trendRegress(win);
    var lastTotal = pend[pend.length - 1];
    var nextIdx = series.length;
    var rawNext = tWin.m * nextIdx + tWin.b;
    var forecastPend = Math.max(0, rawNext);
    // band kasar 95% (normal approx, df terbatas) dari residu fit window
    var sse = 0, sst = 0, mu = win.reduce(function (s, v) { return s + v; }, 0) / win.length;
    for (var i2 = 0; i2 < win.length; i2++) {
      var fit = tWin.m * i2 + tWin.b;
      sse += (win[i2] - fit) * (win[i2] - fit);
      sst += (win[i2] - mu) * (win[i2] - mu);
    }
    var df = win.length - 2 > 0 ? win.length - 2 : 1;
    var sigma = Math.sqrt(sse / df);
    var denomT = tWin.n * tWin.sxx - tWin.sx * tWin.sx;
    var sePred = sigma * Math.sqrt(1 + 1 / tWin.n + Math.pow(nextIdx - tWin.sx / tWin.n, 2) / (denomT || 1));
    var half = 1.96 * sePred;
    var low = Math.max(0, forecastPend - half);
    var high = forecastPend + half;
    var r2 = sst > 0 ? 1 - sse / sst : 0;
    var growth = lastTotal > 0 ? (forecastPend - lastTotal) / lastTotal : 0;
    var note = tWin.m < 0 ? ' Tren menurun — proyeksi lebih rendah dari periode terakhir.' : ' Tren naik — proyeksi mengikuti kemiringan periode terakhir.';
    return Promise.resolve({
      ...
      trend: { m: tWin.m, r2: +r2.toFixed(3), window: w, note: note },
      forecast: { key: 'Berikutnya', pendapatan: +forecastPend.toFixed(0) },
      confidence: { level: 95, low: +low.toFixed(0), high: +high.toFixed(0) },
  ...
  // ---------- parse CSV impor (bulk) ----------
  // Baris pertama = header. Alias Indonesia/Inggris diterjemahkan ke field produk.
  var CSV_ALIASES = {
    nama: ['nama', 'nama produk', 'name', 'produk', 'product'],
    harga: ['harga', 'harga produk', 'price', 'harga jual'],
    komisi: ['komisi', 'komisi%', 'komisi (%)', 'commission', 'comm'],
    klik: ['klik', 'clicks', 'click'],
    konversi: ['konversi', 'penjualan', 'sales', 'conversions', 'conversion'],
    pendapatan: ['pendapatan', 'revenue', 'income', 'omzet'],
    biaya: ['biaya', 'biaya iklan', 'cost', 'costs', 'pengeluaran', 'expense'],
    niche: ['niche', 'kategori', 'category'],
    konten: ['konten', 'format', 'format konten', 'content'],
    platform: ['platform', 'platform konten', 'channel'],
    tanggal: ['tanggal', 'tanggal mulai', 'date', 'bulan']
  };
  function normTok(s) { return String(s || '').toLowerCase().replace(/[^a-z0-9]/g, ''); }
  function csvFieldIndex(header) {
    var map = {};
    header.forEach(function (h, i) {
      ...
      Object.keys(CSV_ALIASES).forEach(function (key) {
        ...
        var ok = CSV_ALIASES[key].some(function (alias) { return normTok(alias) === t; });
        if (ok) map[key] = i;
      });
    });
    return map;
  ...
  function parseCSVRows(text) {
    /* Parser CSV mini: dukung kutip dobel, koma di dalam kutip, dan baris
       multibaris dalam kutip. Baris kosong dilewati. */
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      if (inQ) {
        ...
        cur.push(field); field = '';
        if (cur.some(function (c) { return String(c).replace(/\r/g, '').trim() !== ''; })) rows.push(cur);
        cur = [];
      } else {
        field += ch;
      }
    ...
    if (field !== '' || cur.length) {
      cur.push(field);
      if (cur.some(function (c) { return String(c).trim() !== ''; })) rows.push(cur);
    } else if (rows.length === 0 && text.trim() === '') {
      return { rows: [] };
    }
    if (inQ) return { error: 'Tanda kutip dobel tidak ditutup di baris ' + (rows.length + 1) };
    return { rows: rows };
  }
  function csvMoney(s) {
  ...
  function parseAffCSV(text) {
    var pc = parseCSVRows(text);
    if (pc.error) return { ok: false, error: pc.error };
    var rows = pc.rows;
    if (!rows.length) return { ok: true, rows: [] };
    var header = rows[0].map(function (h) { return String(h).trim(); });
    var idx = csvFieldIndex(header);
    if (idx.nama === undefined) {
      return { ok: false, error: 'Kolom `nama` tidak ditemukan — baris pertama harus header (mis. nama,harga,komisi,klik,konversi,pendapatan,biaya,...).' };
    }
    var out = [];
    for (var r = 1; r < rows.length; r++) {
      var cells = rows[r];
      var get = function (k) { return (idx[k] !== undefined && cells[idx[k]] !== undefined) ? String(cells[idx[k]]).trim() : ''; };
      var nama = get('nama');
      if (!nama) return { ok: false, error: 'Baris ' + (r + 1) + ' tanpa nama produk.' };
      out.push({
        nama: nama,
        niche: get('niche'),
        konten: get('konten'),
        ...
        tanggal: get('tanggal'),
        harga: csvMoney(get('harga')),
        komisiPct: csvMoney(get('komisi')),
        klik: csvInt(get('klik')),
        konversi: csvInt(get('konversi')),
        pendapatan: csvMoney(get('pendapatan')),
        biaya: csvMoney(get('biaya'))
      });
    }
  ...
  function formatForecast(f) {
    ...
    var band = (f.confidence ? ' · kisaran ' + fmtRupiah(f.confidence.low) + '–' + fmtRupiah(f.confidence.high) + ' (C.I. 95% kasar)' : '');
    return '## 🔮 Forecast Pendapatan (per ' + f.period + ')\n\n' +
      '| Periode | Pendapatan | Biaya | Klik | Konversi |\n|---|---:|---:|---:|---:|\n' + rows +
      '\n\n**Proyeksi periode berikutnya:** ' + fmtRupiah(f.forecast.pendapatan) + band +
      ' (_tren ' + (f.growthPct >= 0 ? '+' : '') + f.growthPct + '%, fit R²=' + f.trend.r2 + '_)' +
      (f.trend.note ? '\n' + f.trend.note : '') +
      '\n\n> C.I. kasar dari residu fit ' + f.trend.w + ' periode terakhir. Tambah data/anggarkan biaya iklan utk mempertajam angka.';
  }
