...
(function () {
  ...
  var SHORT_NAMES = { rsi: 'RSI', bb: 'Bollinger Bands', sma: 'SMA Cross', all: 'RSI+MACD+BB' };
  ...
  var STRATEGIES = ['rsi', 'bb', 'sma', 'ema', 'vwap', 'ma', 'smc', 'cvd', 'all'];
  function backtest(data, strategy, params) {
    if (!data || data.length < 60) return { error: 'Data terlalu sedikit untuk backtest' };
    strategy = (strategy || '').toLowerCase();
    if (STRATEGIES.indexOf(strategy) === -1) return { error: 'Strategi tak dikenal. Pilih: rsi/bb/sma/ema/vwap/ma/smc/cvd/all' };
  ...
  // ===== REGIME FILTER (adopsi "RSI Regime Filter" ala LuxAlgo) =====
  ...
  function analyzeConfluence(data, strategy, params, higherTfData) {
    ...
    if (primary === 'flat') {
      var p = indicatorBias(data, strategy === 'ema' ? 'ema' : strategy === 'sma' || strategy === 'ma' ? 'sma' : 'rsi');
      if (p) primary = p;
    }
