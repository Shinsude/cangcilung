...
(function () {
  ...
  function formatSignalAlerts() {
    ...
    var out = '## Live Signal Aktif (' + _signals.length + ')\n';
  ...
  /* Cek data terbaru terhadap daftar sinyal. Kembalikan array sinyal yang BARU
     memunculkan sinyal (fire hanya sekali per state per sinyal). */
  function checkSignalAlerts(data) {
    var fired = [];
    var rows = (data && data.data) || data;
    var sym = (data && data.symbol) || (rows && rows.symbol) || null;
    if (!rows || rows.length < 40) return { signals: _signals.slice(), fired: fired };
    _signals = _signals.filter(function (s) {
      ...
      s.lastIndex = lastIdx;
      s.lastSignal = cur === 'flat' ? null : cur;
      if ((cur === 'long' || cur === 'short') && prev !== cur) {
    ...
    return { signals: _signals.slice(), fired: fired };
  }
