...
(function () {
  ...
  function proxyUrl(target) {
    ...
    if (typeof location !== 'undefined' && location.origin) {
      list.push(location.origin + '/api/quote?url=');
    } else {
      list.push('https://cangcilung.vercel.app/api/quote?url=');
    }
    list.push('DIRECT');
  ...
  function proxyFetch(target, timeoutMs) {
    ...
    var idx = 0;
    function attempt() {
      if (idx >= list.length) return Promise.reject(new Error('Semua sumber data gagal'));
      var mode = list[idx++];
      var url, label;
      if (mode === 'DIRECT') { url = target; label = 'langsung'; }
      else { url = mode + encodeURIComponent(target); label = mode.slice(8, 22); }
      return fetch(url, { signal: AbortSignal.timeout(timeoutMs || 12000) })
        .then(function (r) {
  ...
  function memoCalc(key, data, fn) {
    var sig = '' + data.length + '|' + data[data.length - 1].close + '|' + data[data.length - 1].time + '|' + data.length;
    var k = key + '|' + sig;
  ...
  function fetchWithFallback(symbol, interval) {
    return fetchYahoo(symbol, interval).catch(function (err) {
      var sym = resolveSymbol(symbol);
      var iv = INTERVAL_MAP[interval] || '1d';
      return fetchStooq(sym, iv);
    });
  ...
  function fetchNewsSentiment(symbol, opts) {
    ...
    var ddgUrl = 'https://api.allorigins.win/raw?url=' + encodeURIComponent('https://lite.duckduckgo.com/lite/?q=' + encodeURIComponent(friendly + ' news today') + '&kl=id-id');
    var sources = [];
    var out = [];

    function runSearch() {
      return fetch(ddgUrl, { signal: AbortSignal.timeout(12000) })
    ...
    if (key) {
      var apiUrl = 'https://api.allorigins.win/raw?url=' + encodeURIComponent('https://gnews.io/api/v4/search?q=' + encodeURIComponent(query) + '&lang=en&max=8&apikey=' + encodeURIComponent(key));
      return fetch(apiUrl, { signal: AbortSignal.timeout(12000) })
