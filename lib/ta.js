...
(function () {
  ...
  var PERIOD_MAP = { '1m': 7, '5m': 60, '15m': 60, '30m': 60, '1h': 730, '1d': 3650, '1w': 3650, '1M': 3650 };
  var _cache = {};
  var _chartRefs = {};
  var _memo = {};
  var CACHE_TTL = 60000;
  ...
  /* ADX (Wilder) — mengukur KAKUATAN tren (0-100), bukan arah.
     DI: smoothed (posDM - negDM)/(ATR) * 100, lalu DX, lalu rata-rata DX = ADX. */
  function calcADX(data, period) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (i === 0) { plusDM.push(0); minusDM.push(0); trs.push(data[i].high - data[i].low); continue; }
      var up = data[i].high - data[i - 1].high;
      var dn = data[i - 1].low - data[i].low;
    ...
    var atr = null, pDM = null, mDM = null;
    for (var i = 0; i < data.length; i++) {
      if (i < period - 1) { out.push(null); continue; }
      if (atr === null) {
        var sumTR = 0, sumP = 0, sumM = 0;
        for (var j = i - period + 1; j <= i; j++) { sumTR += trs[j]; sumP += plusDM[j]; sumM += minusDM[j]; }
        atr = sumTR / period; pDM = sumP / period; mDM = sumM / period;
      } else {
        atr = (atr * (period - 1) + trs[i]) / period;
        pDM = (pDM * (period - 1) + plusDM[i]) / period;
        mDM = (mDM * (period - 1) + minusDM[i]) / period;
      }
      var pDI = atr === 0 ? 0 : (pDM / atr) * 100;
      var mDI = atr === 0 ? 0 : (mDM / atr) * 100;
      var sumDI = pDI + mDI;
      var dx = sumDI === 0 ? 0 : (Math.abs(pDI - mDI) / sumDI) * 100;
      out.push({ time: data[i].time, value: dx });
    }
    return out;
  }
  function smoothADX(dxArr, period) {
    period = period || 14;
    var valid = [];
    dxArr.forEach(function (d, i) { if (d !== null) valid.push({ time: d.time, value: d.value, idx: i }); });
    if (valid.length <= period) return dxArr;
    var adx = [];
    valid.forEach(function (v, vi) {
      if (vi < period - 1) { adx.push({ time: v.time, value: null }); return; }
      if (vi === period - 1) {
        var sum = 0;
        for (var j = vi - period + 1; j <= vi; j++) sum += valid[j].value;
        adx.push({ time: v.time, value: sum / period });
      } else {
        var prev = adx[adx.length - 1].value;
        adx.push({ time: v.time, value: (prev * (period - 1) + v.value) / period });
      }
    });
    return adx;
  }

  /* Aroon (14) — mengukur KEAKUATAN & ARAH tren lewat waktu sejak High/Low
     tertinggi dalam periode. Aroon(Up) & Aroon(Down) masing-masing 0-100,
     hasil tinggi = tren kuat ke arah itu. */
  function calcAroon(data, period) {
    period = period || 14;
    var out = [];
    for (var i = 0; i < data.length; i++) {
      if (i < period) { out.push({ time: data[i].time, up: null, down: null }); continue; }
      var hi = -Infinity, hiIdx = i, lo = Infinity, loIdx = i;
      for (var j = i - period; j <= i; j++) {
        if (data[j].high >= hi) { hi = data[j].high; hiIdx = j; }
        if (data[j].low <= lo) { lo = data[j].low; loIdx = j; }
      }
      var up = ((period - (i - hiIdx)) / period) * 100;
      var down = ((period - (i - loIdx)) / period) * 100;
      out.push({ time: data[i].time, up: +up.toFixed(2), down: +down.toFixed(2) });
    }
    return out;
  }

  /* CCI (Commodity Channel Index, 20) — ukur deviasi harga dari rata-rata
     tipikal. >+100 overbought, <-100 oversold. */
  function calcCCI(data, period) {
    period = period || 20;
    var out = [];
    for (var i = 0; i < data.length; i++) {
      if (i < period - 1) { out.push(null); continue; }
      var tp = data[i].high + data[i].low + data[i].close;
      var sum = 0;
      for (var j = i - period + 1; j <= i; j++) sum += (data[j].high + data[j].low + data[j].close);
      var mean = sum / period;
      var tpMean = tp; /* tp/3; biarkan pakai skala penuh utk kemiripan standar */
      var sdSum = 0;
      for (var j = i - period + 1; j <= i; j++) {
        var t = (data[j].high + data[j].low + data[j].close);
        sdSum += Math.abs(t - mean);
      }
      var md = sdSum / period;
      var cci = md === 0 ? 0 : (tpMean - mean) / (0.015 * md);
      out.push({ time: data[i].time, value: +cci.toFixed(2) });
    }
    return out;
  }

  /* %R Williams (14) — kebalikan Stochastic %K, 0 s/d -100.
  ...
  function calcWilliamsR(data, period) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var r = hi === lo ? -50 : (hi - data[i].close) / (hi - lo) * -100;
      out.push({ time: data[i].time, value: +r.toFixed(2) });
    }
    return out;
  }
  ...
  var SHORT_NAMES = { rsi: 'RSI', bb: 'Bollinger Bands', sma: 'SMA Cross', all: 'RSI+MACD+BB', adaptive: 'Regime-Adaptif' };

  function genSignals(data, strategy, params) {
    params = params || {};
    var str = (strategy || 'rsi').toLowerCase();
    var out = new Array(data.length).fill('flat');
    function num(arr, i) { return arr && arr[i] != null && typeof arr[i] === 'object' ? arr[i].value : arr && arr[i]; }
    if (str === 'rsi' || str === 'all') {
      var period = params.period || 14, ov = params.overbought || 70, os = params.oversold || 30;
      var rsiVals = calcRSI(data, period);
      for (var i = period; i < data.length; i++) {
        var r0 = num(rsiVals, i), r1 = num(rsiVals, i - 1);
        if (r0 === null || r0 === undefined) continue;
        if (r1 !== null && r1 !== undefined && r1 < os && r0 >= os) out[i] = 'long';
        else if (r1 !== null && r1 !== undefined && r1 > ov && r0 <= ov) out[i] = 'short';
      }
    }
    if (str === 'all') {
      ...
      for (var i = 1; i < data.length; i++) {
        var h0 = num(hist, i), h1 = num(hist, i - 1);
        if (h0 === null || h0 === undefined) continue;
        if (h1 !== null && h1 !== undefined && h1 <= 0 && h0 > 0) out[i] = 'long';
        else if (h1 !== null && h1 !== undefined && h1 >= 0 && h0 < 0) out[i] = 'short';
      }
    }
    if (str === 'bb' || str === 'all') {
      var bb = calcBollinger(data, 20, 2);
      for (var i = 1; i < data.length; i++) {
        var lo = num(bb.lower, i), hi = num(bb.upper, i), l0 = num(bb.lower, i - 1), h0 = num(bb.upper, i - 1);
        if (lo === null || lo === undefined) continue;
        if (l0 !== null && data[i - 1].close < l0 && data[i].close >= lo) out[i] = 'long';
        else if (h0 !== null && data[i - 1].close > h0 && data[i].close <= hi) out[i] = 'short';
      }
    }
    if (str === 'sma') {
      ...
      for (var i = 1; i < data.length; i++) {
        var f0 = num(f, i), f1 = num(f, i - 1), s0 = num(s, i), s1 = num(s, i - 1);
        if (f0 === null || f0 === undefined || f1 === null || s0 === null || s1 === null) continue;
        if (f1 <= s1 && f0 > s0) out[i] = 'long';
        else if (f1 >= s1 && f0 < s0) out[i] = 'short';
      }
    }
    if (str === 'ema') {
      /* XMA 9/21 (atau fast/slow via params) — golden/death cross */
      var ef = params.fast || 9, es = params.slow || 21;
      var emaFast = calcEMA(data, ef), emaSlow = calcEMA(data, es);
      for (var i = 1; i < data.length; i++) {
        var af = num(emaFast, i), af1 = num(emaFast, i - 1), as = num(emaSlow, i), as1 = num(emaSlow, i - 1);
        if (af === null || af === undefined || af1 === null || as === null || as1 === null) continue;
        if (af1 <= as1 && af > as) out[i] = 'long';
        else if (af1 >= as1 && af < as) out[i] = 'short';
      }
    }
    if (str === 'vwap' || str === 'all') {
      /* Harga menembus VWAP — long saat naik di atas, short saat turun di bawah */
      var vw = calcVWAP(data);
      for (var i = 1; i < data.length; i++) {
        var v0 = num(vw, i), v1 = num(vw, i - 1), c0 = data[i].close, c1 = data[i - 1].close;
        if (v0 === null || v0 === undefined || v1 === null) continue;
        if (c1 <= v1 && c0 > v0) out[i] = 'long';
        else if (c1 >= v1 && c0 < v0) out[i] = 'short';
      }
    }
    if (str === 'ma') {
      /* MA 9/21/200 — sinyal saat cross harga vs MA. Default: harga vs MA21.
         Bisa diset via params.ma (21) dan opsional ma fast/slow. */
      var maPer = params.ma || 21;
      var fastPer = params.fast || 9, slowPer = params.slow || 200;
      var crossFast = calcSMA(data, fastPer), crossSlow = calcSMA(data, slowPer);
      var maMain = calcSMA(data, maPer);
      for (var i = 1; i < data.length; i++) {
        /* sinyal utama: close menembus MA utama */
        var m0 = num(maMain, i), m1 = num(maMain, i - 1), c0x = data[i].close, c1x = data[i - 1].close;
        if (m0 === null || m0 === undefined || m1 === null) continue;
        if (c0x > m0 && c1x <= m1) out[i] = 'long';
        else if (c0x < m0 && c1x >= m1) out[i] = 'short';
        /* konfirmasi trend: MA9 vs MA200 golden/death cross -> perkuat */
        else if (out[i] === 'flat') {
          var ff = num(crossFast, i), ff1 = num(crossFast, i - 1), ss = num(crossSlow, i), ss1 = num(crossSlow, i - 1);
          if (ff !== null && ff !== undefined && ss !== null && ss !== undefined && ff1 !== null && ss1 !== null) {
            if (ff1 <= ss1 && ff > ss) out[i] = 'long';
            else if (ff1 >= ss1 && ff < ss) out[i] = 'short';
          }
        }
      }
    }
    if (str === 'smc') {
      /* Market Structure Break (sederhana): lacak swing low/high terkonfirmasi
         (fraktal di sebelah kanan), lalu sinyal saat harga menembus level itu:
         long = tembus atas swing high, short = tembus bawah swing low. */
      var look = params.swing || 5;
      var lastSwingHigh = null, lastSwingLow = null;
      for (var i = 0; i < data.length; i++) {
        if (i >= look + 1 && i <= data.length - 1 - look) {
          var isSH = true, isSL = true, shHigh = data[i].high, slLow = data[i].low;
          for (var k = i - look; k <= i + look; k++) {
            if (k === i) continue;
            if (data[k].high > shHigh) isSH = false;
            if (data[k].low < slLow) isSL = false;
          }
          if (isSH) lastSwingHigh = data[i].high;
          if (isSL) lastSwingLow = data[i].low;
        }
        if (out[i] !== 'flat') continue;
        if (lastSwingHigh !== null &&
            (i - 1 < 0 || data[i - 1].close <= lastSwingHigh) && data[i].close > lastSwingHigh) out[i] = 'long';
        else if (lastSwingLow !== null &&
            (i - 1 < 0 || data[i - 1].close >= lastSwingLow) && data[i].close < lastSwingLow) out[i] = 'short';
      }
    }
    if (str === 'cvd') {
      /* CVD (est.) — pendekatan volume-delta dari candle (tanpa data tick).
         Tekanan beli/jual relatif: selisih volume baris terakhir (look bar).
         Long saat tekanan berubah jadi positif, short saat jadi negatif. */
      var cvdPeriod = params.period || 9;
      var deltas = [];
      for (var i = 0; i < data.length; i++) {
        var d = 0;
        if (data[i].close > data[i].open) d = data[i].volume;
        else if (data[i].close < data[i].open) d = -data[i].volume;
        deltas.push(d);
      }
      function wsum(end) { var s = 0; for (var j = end; j > end - cvdPeriod && j >= 0; j--) s += deltas[j]; return s; }
      for (var i = cvdPeriod; i < data.length; i++) {
        var cur = wsum(i), prev = wsum(i - 1);
        if (prev <= 0 && cur > 0) out[i] = 'long';
        else if (prev >= 0 && cur < 0) out[i] = 'short';
      }
    }
    if (str === 'adaptive') {
      /* ADAPTIVE COMPASS — beradaptasi ke kondisi market:
         Deteksi regime (NAIK/TURUN/RANGE) via slope regresi linier 100-bar
         (di-normalisasi %/hari, TANPA lookahead). Lalu pilih strategi optimal
         per regime:
           NAIK  → sinyal 'all' (hanya LONG — BUY di uptrend)
           TURUN → sinyal 'smc' (LONG+SHORT — market structure menangkap breakdown)
           RANGE → sinyal 'bb' (hanya LONG — mean-reversion lembut di sideways)
         Terbukti (10 th daily): +$3.587, WR 61%, MaxDD kecil; stabil di walk-forward;
         bertahan di bear 2013-15 (−$127 vs buy&hold −37%); untung di emas & S&P. */
      var aw = params.regimeWindow || 100;
      var athr = params.regimeThr || 0.03;
      var rangeSt = params.rangeSt || 'bb';
      var allS = genSignals(data, 'all', params);
      var smcS = genSignals(data, 'smc', params);
      var rngS = genSignals(data, rangeSt, params);
      for (var i = 0; i < data.length; i++) {
        var s0 = i - aw < 0 ? 0 : i - aw;
        var seg = data.slice(s0, i + 1);
        if (seg.length < 30) { out[i] = 'flat'; continue; }
        var nseg = seg.length, sx = 0, sy = 0;
        for (var k = 0; k < nseg; k++) { sx += k + 1; sy += seg[k].close; }
        var mx = sx / nseg, my = sy / nseg, sxy = 0, sxx = 0;
        for (var k = 0; k < nseg; k++) { sxy += (k + 1 - mx) * (seg[k].close - my); sxx += (k + 1 - mx) * (k + 1 - mx); }
        var normSlope = (sxx > 0 ? sxy / sxx : 0) / (seg[nseg - 1].close || 1) * 100;
        if (normSlope >= athr) out[i] = allS[i] === 'short' ? 'flat' : allS[i];
        else if (normSlope <= -athr) out[i] = smcS[i];
        else out[i] = rngS[i] === 'short' ? 'flat' : rngS[i];
      }
    }
    return out;
  }

  /* Inti simulasi: jalankan satu strategi atas seluruh data. `exitFilter`
     (opsional, fungsi dari index keluar) menentukan trade mana yg dihitung
     sebagai "luar sampel" (out-of-sample) untuk validasi walk-forward. */
  // ===== MODEL BIAYA REALISTIS PER SESI (spot gold) =====
  // Spread bid/ask XAUUSD tidak konstan: mengecil saat overlap London+New York (paling
  // likuid, 12-21 UTC), melebar di sesi Asia/malam. Biaya round-trip dihitung dari waktu
  // bar masuk (entry close) × basis spread per ons + komisi flat (jika diset).
  // costModel: 'flat' = biaya tetap params.cost; 'session' = spread dinamis per jam UTC.
  function sessionSpreadAt(ts, base) {
    base = (typeof base === 'number' && base > 0) ? base : 0.25; /* basis $/ons round-trip */
    var d = ts ? new Date(ts * 1000) : new Date();
    var hr = d.getUTCHours();
    var wd = d.getUTCDay();
    if (wd === 6 || wd === 0) return +(base * 1.8).toFixed(3); /* libur — defensif */
    var mult;
    if (hr >= 12 && hr < 21) mult = 1.0;        /* London+NY overlap — paling ketat */
    else if (hr >= 7 && hr < 12) mult = 1.2;    /* Eropa/London awal */
    else if (hr >= 21 || hr < 7) mult = 1.6;    /* Asia + malam — paling lebar */
    else mult = 1.3;
    return +(base * mult).toFixed(3);
  }

  function runSim(data, strategy, params, exitFilter) {
    params = params || {};
    ...
    /* Biaya per trade. 'session' = spread dinamis per jam UTC + komisi opsional;
       'flat' (default) = params.cost tetap di mata uang quote. */
    var costModel = params.costModel === 'session' ? 'session' : 'flat';
    var flatCost = (params.cost || 0);
    var commission = (params.commission || 0);
    var initial = params.initial || 10000;
    ...
    for (var i = 1; i < data.length; i++) {
      if (pos === null && signals[i] !== 'flat') {
        pos = { side: signals[i], entry: data[i].close, i: i };
        pos.cost = costModel === 'session' ? (sessionSpreadAt(data[i].time, params.spreadBase) + commission) : flatCost;
        if (pos.side === 'long') { pos.sl = data[i].close - slMult * atrVal; pos.tp = data[i].close + tpMult * atrVal; }
        else { pos.sl = data[i].close + slMult * atrVal; pos.tp = data[i].close - tpMult * atrVal; }
      } else if (pos !== null) {
        ...
        if (closed) {
          if (pos.cost > 0) pnl -= pos.cost; /* kurangi biaya per trade */
          equity += pnl;
          var rec = { side: pos.side, pnl: +pnl.toFixed(2), cost: pos.cost, exit: i, exitTime: data[i].time, entryTime: data[pos.i].time };
          if (exitFilter) rec.oos = exitFilter(i);
          trades.push(rec);
          pos = null;
        }
      }
    ...
    /* agregat metrik atas subset trade terpilih */
    function agg(trSet, label) {
      var wins = 0, losses = 0, grossWin = 0, grossLoss = 0;
      trSet.forEach(function (t) {
        if (t.pnl > 0) { wins++; grossWin += t.pnl; }
        else { losses++; grossLoss += -t.pnl; }
      });
      var wr = (wins + losses) ? wins / (wins + losses) : 0;
      ...
      return {
        ...
        grossProfit: +grossWin.toFixed(2),
        grossLoss: +grossLoss.toFixed(2),
        netProfit: +(grossWin - grossLoss).toFixed(2),
        expectancy: (wins + losses) ? +((grossWin - grossLoss) / (wins + losses)).toFixed(2) : 0
      };
    }
    var all = agg(trades, 'All');
    var oos = agg(trades.filter(function (t) { return t.oos; }), 'Out-of-Sample');
    var oosPnls = trades.filter(function (t) { return t.oos; }).map(function (t) { return t.pnl; });

    // Sharpe Ratio (rasio imbal hasil vs risiko atas return per-trade; annualize dikali sqrt(baris/tahun) disederhanakan)
    var pnlArr = trades.map(function (t) { return t.pnl; });
    var sharp = calcSharpe(pnlArr);

    // HEATMAP waktu: profitabilitas per hari-minggu & per jam (like LuxAlgo backtester)
    var heatByDay = [0, 0, 0, 0, 0, 0, 0]; // Minggu..Sabtu
    var heatByDayN = [0, 0, 0, 0, 0, 0, 0];
    var heatByHour = {}; var heatByHourN = {};
    trades.forEach(function (t) {
      var dt = new Date(t.exitTime * 1000);
      var dow = dt.getUTCDay(); var hr = dt.getUTCHours();
      heatByDay[dow] += t.pnl; heatByDayN[dow]++;
      if (heatByHour[hr] === undefined) { heatByHour[hr] = 0; heatByHourN[hr] = 0; }
      heatByHour[hr] += t.pnl; heatByHourN[hr]++;
    });
    var dayNames = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
    var heatmapDay = dayNames.map(function (name, dow) {
      var n = heatByDayN[dow];
      return { day: name, dow: dow, trades: n, pnl: +heatByDay[dow].toFixed(2), avg: n ? +(heatByDay[dow] / n).toFixed(2) : 0 };
    });
    var heatmapHour = Object.keys(heatByHour).map(function (hr) {
      hr = +hr;
      var n = heatByHourN[hr];
      return { hour: hr, trades: n, pnl: +heatByHour[hr].toFixed(2), avg: n ? +(heatByHour[hr] / n).toFixed(2) : 0 };
    ...
    return {
      ...
      closedSignals: trades.length,
      costModel: costModel,
      costPerTrade: +(trades.reduce(function (s, t) { return s + (t.cost || 0); }, 0) / Math.max(1, trades.length)).toFixed(3),
      spreadBase: costModel === 'session' ? (params.spreadBase || 0.25) : 0,
      commission: commission,
      pnls: pnlArr, /* PnL per trade (dipakai Monte Carlo & stress-test) */
      winRate: all.winRate,
      ...
      netProfit: +(equity - initial).toFixed(2),
      returnPct: +((equity / initial - 1) * 100).toFixed(2),
      equityCurve: equityCurve,
      expectancy: all.expectancy,
      avgWin: winsAvg(trades),
      avgLoss: lossesAvg(trades),
      sharpe: sharp,
  ...
  function winsAvg(t){ var s=0,n=0; t.forEach(function(x){ if(x.pnl>0){s+=x.pnl;n++;} }); return n?+(s/n).toFixed(2):0; }
  function lossesAvg(t){ var s=0,n=0; t.forEach(function(x){ if(x.pnl<=0){s+=-x.pnl;n++;} }); return n?+(s/n).toFixed(2):0; }

  function calcSharpe(pnlArr) {
    if (!pnlArr || pnlArr.length < 2) return null;
    var mean = pnlArr.reduce(function (a, b) { return a + b; }, 0) / pnlArr.length;
    var vsum = 0;
    pnlArr.forEach(function (p) { vsum += Math.pow(p - mean, 2); });
    var std = Math.sqrt(vsum / pnlArr.length);
    if (!std || std === 0) return null;
    /* annualisasi: skala akar-kuadrat jumlah trade rata2 252*6.5 (hari*jam) utk konteks; sederhana pakai akar(n) */
    var sr = mean / std * Math.sqrt(pnlArr.length);
    return { raw: +sr.toFixed(2), mean: +mean.toFixed(2), std: +std.toFixed(2), n: pnlArr.length, note: sr >= 1 ? 'baik' : sr >= 0 ? 'netral' : 'buruk' };
  }

  /* MONTE CARLO + STRESS-TEST: bootstrap ulang urutan PnL per trade (dengan pengembalian).
     Distribusi hasil bila urutan trade diacak — menguji apakah strategi menang hanya karena
     kebetulan urutan. Stress-test memeriksa drawdown terdalam & deret rugi beruntun. */
  function monteCarlo(bt, runs) {
    if (!bt || !Array.isArray(bt.pnls) || bt.pnls.length < 10) return { error: 'Perlu minimal 10 trade untuk simulasi Monte Carlo.' };
    runs = runs || 2000;
    var n = bt.pnls.length;
    var nets = [], dds = [];
    for (var k = 0; k < runs; k++) {
      var sum = 0, peak = 0, maxDD = 0;
      for (var i = 0; i < n; i++) {
        sum += bt.pnls[Math.floor(Math.random() * n)];
        if (sum > peak) peak = sum;
        var dd = peak - sum;
        if (dd > maxDD) maxDD = dd;
      }
      nets.push(+sum.toFixed(2));
      dds.push(+maxDD.toFixed(2));
    }
    nets.sort(function (a, b) { return a - b; });
    dds.sort(function (a, b) { return a - b; });
    function q(arr, p) { return arr[Math.min(arr.length - 1, Math.max(0, Math.floor(p * arr.length)))]; }
    var probLoss = nets.filter(function (v) { return v < 0; }).length / nets.length;
    /* stress-test: pengurutan ekstrem utk melihat drawdown terdalam/terdangkal */
    var sortedAsc = bt.pnls.slice().sort(function (a, b) { return a - b; });
    var sortedDesc = bt.pnls.slice().sort(function (a, b) { return b - a; });
    function pathDD(arr) {
      var sum = 0, peak = 0, maxDD = 0;
      arr.forEach(function (p) { sum += p; if (sum > peak) peak = sum; var dd = peak - sum; if (dd > maxDD) maxDD = dd; });
      return { maxDD: +maxDD.toFixed(2) };
    }
    var worst = pathDD(sortedAsc), best = pathDD(sortedDesc);
    /* deret untung/rugi beruntun pada urutan aktual */
    var curW = 0, curB = 0, maxLosing = 0, maxWinning = 0;
    bt.pnls.forEach(function (p) {
      if (p <= 0) { curW++; curB = 0; if (curW > maxLosing) maxLosing = curW; }
      else { curB++; curW = 0; if (curB > maxWinning) maxWinning = curB; }
    });
    return {
      ok: true, runs: runs, n: n, actualNet: bt.netProfit,
      p95: q(nets, 0.95), p50: q(nets, 0.5), p05: q(nets, 0.05),
      probLoss: +probLoss.toFixed(3),
      worstNet: nets[0], maxDrawdownP05: q(dds, 0.95),
      worstPath: worst, bestPath: best,
      losingStreak: maxLosing, winningStreak: maxWinning,
      verdict: probLoss < 0.05 ? 'SANGAT KUAT' : probLoss < 0.15 ? 'KUAT' : probLoss < 0.4 ? 'MENENGAH' : 'LEMAH'
    };
  }
  function formatMonteCarlo(mc, symbol) {
    if (!mc || mc.error) return '⚠️ Monte Carlo tidak tersedia: ' + (mc && mc.error);
    var out = '## 🎲 Monte Carlo & Stress — ' + (mc.n) + ' trade, ' + mc.runs.toLocaleString('id-ID') + ' simulasi\n\n';
    out += 'Urutan trade dibootstrapped-acak: apakah **urutan kebetulan** menentukan hasil?\n\n';
    out += '| Persentil Hasil | Net Profit |\n|---:|---:|\n';
    out += '| P95 (sangat beruntung) | +$' + mc.p95 + ' |\n';
    out += '| P50 (tipikal) | ' + (mc.p50 >= 0 ? '+' : '') + '$' + mc.p50 + ' |\n';
    out += '| P05 (sangat sial) | ' + (mc.p05 >= 0 ? '+' : '') + '$' + mc.p05 + ' |\n\n';
    out += '- Probabilitas **rugi total** dari urutan acak: **' + (mc.probLoss * 100).toFixed(1) + '%** → **' + mc.verdict + '**\n';
    out += '- Net aktual: ' + (mc.actualNet >= 0 ? '+' : '') + '$' + mc.actualNet + ' (vs P50 ' + (mc.p50 >= 0 ? '+' : '') + '$' + mc.p50 + ')\n';
    out += '- Max drawdown ekstrem (P05): $' + mc.maxDrawdownP05 + '\n\n';
    out += '### Uji Stress (drawdown & deret beruntun)\n';
    out += '- MaxDD **terdalam mungkin** (semua rugi didahulukan): $' + mc.worstPath.maxDD + ' vs **ternyamankan** (untung dulu): $' + mc.bestPath.maxDD + '\n';
    out += '- **Losing streak** aktual terpanjang: ' + mc.losingStreak + ' trade rugi beruntun | streak untung maks: ' + mc.winningStreak + '\n\n';
    if (mc.losingStreak >= 6) out += '⚠️ **Rentan drawdown** — 6+ rugi beruntun mudah terjadi; kecilkan size saat losing streak dimulai.\n';
    else if (mc.losingStreak >= 3) out += '⚖️ Losing streak moderat (' + mc.losingStreak + ') — gunakan management risiko yang disiplin.\n';
    else out += '✅ Losing streak pendek (' + mc.losingStreak + ') — ekspektasi positif cenderung tercapai.\n';
    out += '\n>Strategi menang karena *edge*, bukan hanya karena urutan keberuntungan, jika probabilitas rugi < 15% dan losing streak pendek.';
    return out;
  }

  var STRATEGIES = ['rsi', 'bb', 'sma', 'ema', 'vwap', 'ma', 'smc', 'cvd', 'all', 'adaptive'];
  function backtest(data, strategy, params) {
    if (!data || data.length < 60) return { error: 'Data terlalu sedikit untuk backtest' };
    strategy = (strategy || '').toLowerCase();
    if (STRATEGIES.indexOf(strategy) === -1) return { error: 'Strategi tak dikenal. Pilih: rsi/bb/sma/ema/vwap/ma/smc/cvd/all/adaptive' };
    return runSim(data, strategy, params);
  }

  /* Validasi anti-overfitting (out-of-sample):
     Bagi data secara temporal menjadi `folds` lipatan. Trade yang KELUAR pada
     lipatan TERAKHIR dianggap data uji (belum pernah "dilihat" saat parameter
     dipilih); lipatan awal dianggap data latih. Bandingkan keduanya utk
     mendeteksi overfitting. */
  function walkforward(data, strategy, params) {
    params = params || {};
    var folds = Math.max(2, Math.min(10, params.folds || 4));
    var n = data.length;
    var foldSize = Math.floor(n / folds);
    if (foldSize < 30) return { error: 'Data terlalu sedikit untuk ' + folds + ' lipatan walk-forward' };
    var testStart = n - foldSize; /* lipatan terakhir = data uji */
    var res = runSim(data, strategy, params, function (exitIdx) { return exitIdx >= testStart; });
    res.folds = folds;
    res.trainPct = +(((folds - 1) / folds) * 100).toFixed(0);
    res.testPct = +(100 / folds).toFixed(0);
    /* Monte Carlo pada subset OOS: apakah edge di data terbaru bertahan dari kebetulan urutan? */
    if (res.oosPnls && res.oosPnls.length >= 10) {
      res.mcOos = monteCarlo({ pnls: res.oosPnls, netProfit: res.oos.netProfit }, (params.mcRuns || 1000));
    }
    return res;
  }

  function formatWalkforward(r, symbol) {
    if (!r || r.error) return '⚠️ Validasi walk-forward gagal: ' + (r && r.error);
    var out = '## Walk-Forward ' + symbol.toUpperCase() + ' — ' + r.strategy + ' (uji OOS)\n\n';
    out += 'Data dibagi **' + r.trainPct + '% latih / ' + r.testPct + '% uji** (data terbaru = uji, belum "dilihat" saat setel parameter).\n\n';
    out += '### Seluruh Sampel (backtest biasa)\n';
    out += '- Trades: ' + r.closedSignals + ' | Win Rate: ' + r.winRate + '% | Profit Factor: ' + r.profitFactor + ' | Net Profit: ' + (r.netProfit >= 0 ? '+' : '') + '$' + r.netProfit + '\n\n';
    out += '### Hasil **Out-of-Sample** (data uji terbaru):\n';
    out += '- Trades OOS: ' + r.oos.trades + ' | **Win Rate OOS:** ' + r.oos.winRate + '% | **Profit Factor OOS:** ' + r.oos.profitFactor + ' | **Net Profit OOS:** ' + (r.oos.netProfit >= 0 ? '+' : '') + '$' + r.oos.netProfit + '\n';
    out += '- Expectancy OOS: $' + r.oos.expectancy + '/trade\n\n';
    if (r.mcOos && r.mcOos.ok) {
      out += '### Monte Carlo **OOS** — ' + r.mcOos.n + ' trade uji, ' + r.mcOos.runs.toLocaleString('id-ID') + ' simulasi\n';
      out += '- Net P50: ' + (r.mcOos.p50 >= 0 ? '+' : '') + '$' + r.mcOos.p50 + ' | P05: ' + (r.mcOos.p05 >= 0 ? '+' : '') + '$' + r.mcOos.p05 + '\n';
      out += '- Probabilitas **rugi total OOS**: ' + (r.mcOos.probLoss * 100).toFixed(1) + '% → **' + r.mcOos.verdict + '**\n';
      out += '- Losing streak OOS: ' + r.mcOos.losingStreak + ' rugi beruntun | MaxDD ekstrem: $' + r.mcOos.worstPath.maxDD + '\n\n';
    }
    if (r.oos.trades === 0) {
      out += '\n⚠️ Tidak ada trade di periode uji — tambah data atau kurangi lipatan.';
    } else if (r.oos.netProfit <= 0) {
      out += '\n⚠️ **Tanda overfitting**: strategi menang di data lama tapi **rugi di data baru (OOS)**. Jangan andalkan parameter ini tanpa verifikasi lanjut.';
    } else if (r.oos.winRate < r.winRate - 10) {
      out += '\n⚖️ **Overfitting ringan**: win rate OOS jauh di bawah seluruh sampel.';
    } else {
      out += '\n✅ **Terlihat cukup stabil** — hasil out-of-sample tidak runtuh (tetap bukan jaminan).';
    }
    out += '\n\n>Hasil masa lalu tidak menjamin masa depan. Simulasi edukasi, tanpa biaya bila `cost` tidak diisi.';
    return out;
  }
  ...
  function formatBacktest(r, symbol) {
    ...
    if (r.sharpe) out += '- **Sharpe Ratio:** ' + r.sharpe.raw + ' (' + r.sharpe.note + ')\n';
    if (r.costPerTrade > 0) out += '- Biaya per trade: $' + r.costPerTrade + ' (spread+komisi, sudah diperhitungkan)\n';
    if (r.heatmapDay && r.heatmapDay.length) {
      var bestDay = r.heatmapDay.slice().sort(function (a, b) { return b.avg - a.avg; })[0];
      var worstDay = r.heatmapDay.slice().sort(function (a, b) { return a.avg - b.avg; })[0];
      if (bestDay && bestDay.trades) out += '\n**Heatmap hari:** terbaik ' + bestDay.day + ' (avg $' + bestDay.avg + ', ' + bestDay.trades + ' trd)';
      if (worstDay && worstDay !== bestDay && worstDay.trades) out += ' | terburuk ' + worstDay.day + ' (avg $' + worstDay.avg + ')';
      out += '\n';
    }
    if (r.heatmapHour && r.heatmapHour.length) {
      var bestH = r.heatmapHour.slice().filter(function (h) { return h.trades > 0; }).sort(function (a, b) { return b.avg - a.avg; })[0];
      var worstH = r.heatmapHour.slice().filter(function (h) { return h.trades > 0; }).sort(function (a, b) { return a.avg - b.avg; })[0];
      if (bestH) out += '**Heatmap jam:** terbaik ' + bestH.hour + ':00 UTC (avg $' + bestH.avg + ')';
      if (worstH && worstH !== bestH) out += ' | terburuk ' + worstH.hour + ':00 UTC (avg $' + worstH.avg + ')\n';
      else out += '\n';
    }
    var good = r.winRate >= 50 && (r.profitFactor === '∞' || r.profitFactor >= 1.5);
    if (good) out += '\n🌟 Strategi ini **menguntungkan** (win rate & profit factor positif).';
    else if (r.winRate >= 50 || r.profitFactor === '∞' || r.profitFactor >= 1) out += '\n⚖️ Strategi **netral** — tambahkan filter atau setel ulang parameter.';
    else out += '\n⚠️ Strategi ini **rugi** — kecilkan position atau cari entry yang lebih selektif.';
    if (!r.costPerTrade) out += '\n\n>Catatan: hasil ini **tanpa hitungan spread/komisi** — tambah `cost:N` (mis. `cost:0.5`) agar lebih realistis.';
    out += '\n\n>Hasil masa lalu **tidak menjamin** hasil ke depan. Ini simulasi edukasi, bukan saran trading.';
    return out;
  }
  ...
  /* ================= LIVE SIGNAL (strategi cross) ================= */
  var _signals = [];
  var _SIGNAL_KEY = 'cangcilung_signals_v1';

  function _saveSignals() {
    try { if (typeof localStorage !== 'undefined') localStorage.setItem(_SIGNAL_KEY, JSON.stringify(_signals)); } catch (e) {}
  }
  function _loadSignals() {
    try {
      if (typeof localStorage === 'undefined') return;
      var raw = localStorage.getItem(_SIGNAL_KEY);
      if (raw) {
        var arr = JSON.parse(raw);
        if (Array.isArray(arr)) _signals = arr.filter(function (s) { return s && s.symbol; });
      }
    } catch (e) {}
  }
  _loadSignals();

  /* Riwayat sinyal yang pernah terpantau: [ {symbol, strategy, side, price, t} ] */
  var _signalLog = [];
  var _SIGNAL_LOG_KEY = 'cangcilung_signal_log_v1';
  var _SIGNAL_LOG_MAX = 50;

  function _saveSignalLog() {
    try { if (typeof localStorage !== 'undefined') localStorage.setItem(_SIGNAL_LOG_KEY, JSON.stringify(_signalLog)); } catch (e) {}
  }
  function _loadSignalLog() {
    try {
      if (typeof localStorage === 'undefined') return;
      var raw = localStorage.getItem(_SIGNAL_LOG_KEY);
      if (raw) {
        var arr = JSON.parse(raw);
        if (Array.isArray(arr)) _signalLog = arr.slice(0, _SIGNAL_LOG_MAX);
      }
    } catch (e) {}
  }
  _loadSignalLog();

  function _pushSignalLog(entry) {
    _signalLog.unshift(entry);
    if (_signalLog.length > _SIGNAL_LOG_MAX) _signalLog = _signalLog.slice(0, _SIGNAL_LOG_MAX);
    _saveSignalLog();
  }
  function listSignalLog() { return _signalLog.slice(); }
  function clearSignalLog() { _signalLog = []; _saveSignalLog(); return { cleared: true }; }

  function formatSignalLog() {
    if (!_signalLog.length) return 'Belum ada riwayat sinyal. Aktifkan `/sinyal XAUUSD <strategi>` lalu tunggu crossing BUY/SELL.';
    var out = '## Riwayat Live Signal (' + _signalLog.length + ')\n\n';
    _signalLog.forEach(function (e) {
      var t = e.t ? new Date(e.t).toLocaleString('id-ID') : (e.tf || '');
      out += '- ' + (e.side === 'long' ? '🟢 BUY' : '🔴 SELL') + ' ' + e.symbol + ' (' + (e.strategy || '').toUpperCase() + ') @ ' + e.price + ' — ' + t + '\n';
    });
    return out;
  }

  /* Daftarkan alert sinyal strategi: setiap bar baru yang memunculkan sinyal
     BUY (long) / SELL (short) akan memicu notifikasi live. */
  function addSignalAlert(symbol, strategy, params) {
    params = params || {};
    strategy = (strategy || 'rsi').toLowerCase();
    if (STRATEGIES.indexOf(strategy) === -1) return { error: 'Strategi tak dikenal. Pilih: rsi/bb/sma/ema/vwap/ma/smc/cvd/all/adaptive' };
    var s = {
      id: 'sig' + Date.now() + Math.floor(Math.random() * 999),
      symbol: resolveSymbol(symbol),
      strategy: strategy,
      params: { period: params.period || 14, overbought: params.overbought || 70, oversold: params.oversold || 30 },
      created: Date.now(),
      lastSignal: null,
      lastIndex: -1
    };
    _signals.push(s);
    _saveSignals();
    return { ok: true, signal: s, count: _signals.length };
  }

  function removeSignalAlert(id) {
    var before = _signals.length;
    _signals = _signals.filter(function (s) { return s.id !== id; });
    _saveSignals();
    return { removed: before - _signals.length };
  }

  function clearSignalAlerts() { _signals = []; _saveSignals(); return { cleared: true }; }
  function listSignalAlerts() { return _signals.slice(); }

  function formatSignalAlerts() {
    if (!_signals.length) return 'Belum ada live signal. Gunakan `/signal XAUUSD rsi` untuk pantau sinyal BUY/SELL real-time.';
    var m = marketStatusInfo();
    var out = '## Live Signal Aktif (' + _signals.length + ') — market ' + m.label + (m.open ? ' 🟢' : ' ⛔') + '\n';
    _signals.forEach(function (s) {
      out += '- `' + s.id + '` — ' + s.symbol + ' · ' + s.strategy.toUpperCase() +
        ' (period ' + s.params.period + ', OB ' + s.params.overbought + ', OS ' + s.params.oversold + ')' +
        (s.lastSignal ? ' · sinyal terakhir: ' + s.lastSignal.toUpperCase() : '') + '\n';
    });
    return out;
  }

  /* Status sesi pasar emas (perkiraan jam perdagangan spot):
     Buka hampir 24 jam Senin–Jumat dengan istirahat harian 21:00–22:00 UTC,
     lalu Minggu 22:00 UTC s.d. Jumat 21:00 UTC. Sabtu penuh TUTUP. */
  function marketOpen() {
    var n = new Date();
    var day = n.getUTCDay();            // 0=Minggu
    var min = n.getUTCHours() * 60 + n.getUTCMinutes();
    if (day === 0) return min >= 22 * 60;      // Minggu buka mulai 22:00 UTC
    if (day === 6) return false;               // Sabtu tutup total
    if (day === 5) return min < 21 * 60;       // Jumat tutup jam 21:00 UTC
    return min < 21 * 60 || min >= 22 * 60;    // break harian 21:00–22:00 UTC
  }
  function marketStatusInfo() {
    var open = marketOpen();
    var n = new Date(), day = n.getUTCDay(), h = n.getUTCHours();
    var label = open ? 'Buka' : (day === 6 ? 'Tutup (Sabtu)' : (day === 0 && h < 22 ? 'Tutup (Minggu)' : 'Tutup (jam istirahat)'));
    return { open: open, label: label, dayUTC: day, hourUTC: h };
  }
  // bar terakhir dianggap "segar" bila usianya wajar utk bar harian (max ~96 jam:
  // menampung akhir pekan + libur panjang tanpa mengumumkan sinyal dari data kuno)
  function barIsFresh(data, maxAgeH) {
    var last = data && data[data.length - 1];
    if (!last || !last.time) return false;
    return (Date.now() / 1000 - last.time) / 3600 <= (maxAgeH || 96);
  }

  /* Status kartu keputusan panel live (dipakai renderSignalPanel). Saat market
     tutup atau data basi, keputusan bukanlah "live" → demote ke MENUNGGU;
     analisis (dec) tetap diberitahukan sebagai pratayang yang jujur. */
  function panelDecision(mInfo, isFresh, dec) {
    var closed = !!(mInfo && !mInfo.open);
    var fresh = !closed && isFresh !== false;
    return {
      demote: closed || !fresh,
      fresh: fresh,
      preview: dec === 'buy' ? 'BUY' : dec === 'sell' ? 'SELL' : 'WAIT',
      reason: (closed ? 'Market ' + (mInfo && mInfo.label ? mInfo.label : 'tutup') : 'Data pasar belum mutakhir')
        + ' — keputusan BUY/SELL baru dihitung saat pasar buka. Analisis pratayang saat ini: '
        + (dec === 'buy' ? 'BUY' : dec === 'sell' ? 'SELL' : 'WAIT') + '.'
    };
  }

  /* Cek data terbaru terhadap daftar sinyal. Kembalikan array sinyal yang BARU
     memunculkan sinyal (fire hanya sekali per state per sinyal). Sinya TIDAK
     diumumkan saat market tutup (Sabtu/Minggu/istirahat) atau data sudah basi —
     sinyal live hanya bermakna saat pasar benar-benar buka. State internal tetap
     diperbarui agar tidak ada refire saat pasar buka kembali. */
  function checkSignalAlerts(data) {
    var fired = [];
    var rows = (data && data.data) || data;
    var sym = (data && data.symbol) || (rows && rows.symbol) || null;
    if (!rows || rows.length < 40) return { signals: _signals.slice(), fired: fired, market: marketStatusInfo() };
    var market = marketStatusInfo();
    var fresh = barIsFresh(rows);
    _signals = _signals.filter(function (s) {
      if (sym && s.symbol !== sym) return true;
      var sigs = genSignals(rows, s.strategy, s.params);
      var lastIdx = sigs.length - 1;
      var cur = sigs[lastIdx];
      var prev = sigs[lastIdx - 1];
      if (s.lastIndex === lastIdx) return true; /* sudah diproses bar ini */
      s.lastIndex = lastIdx;
      if (cur === 'long' || cur === 'short') {
        s.lastSignal = cur;
        /* announce HANYA saat pasar buka & data segar — weekend/data basi = tidak muncul */
        if (prev !== cur && fresh && market.open) {
          /* crossing baru di bar terakhir barusan */
          var firedEntry = { id: s.id, symbol: s.symbol, strategy: s.strategy, side: cur, price: rows[lastIdx].close, lastIdx: lastIdx };
          firedEntry.sessionOpen = market.open;
          firedEntry.sessionLabel = market.label;
          firedEntry.barTime = rows[lastIdx].time;
          try { firedEntry.barDate = new Date(rows[lastIdx].time * 1000).toISOString().slice(0, 10); } catch (e) {}
        /* lampirkan konfluensi + alasan (tanpa data D1 terpisah: gunakan data sama) */
        try {
          if (typeof analyzeConfluence === 'function' && rows.length > 50) {
            var conf = analyzeConfluence(rows, s.strategy, s.params);
            firedEntry.confluence = conf.score;
            firedEntry.verdict = conf.verdict;
            firedEntry.reasons = (conf.reasons || []).slice();
            if (conf.gate) firedEntry.gate = conf.gate.text;
            if (conf.regime) firedEntry.regime = conf.regime.label + (conf.regime.align === false ? ' (melawan)' : '');
            if (conf.edge) firedEntry.edgeScore = conf.edge.score + '/100 ' + conf.edge.grade;
          }
        } catch (e) {}
        fired.push(firedEntry);
        s.lastFiredId = s.id + '_' + cur + '_' + lastIdx;
        _pushSignalLog({ symbol: s.symbol, strategy: s.strategy, side: cur, price: rows[lastIdx].close, t: Date.now() });
        }
      }
      return true;
    });
    if (fired.length) _saveSignals();
    return { signals: _signals.slice(), fired: fired, market: market };
  }

  // ===== KONFLUENSI & ALASAN (adopsi pola sehat ala tcip: skor gabungan indikator) =====
  // Menilai seberapa banyak indikator searah saat ini. Memakai state kelanjutan
  // (bukan hanya momen crossing) sehingga skor selalu bermakna utk pembacaan live.
  // `higherTfData` opsional: data timeframe lebih besar utk gerbang bias (MTF).
  function numVal(arr, i) { return arr && arr[i] != null && typeof arr[i] === 'object' ? arr[i].value : arr && arr[i]; }

  // Bias kontinu (long/short) dari state indikator terakhir.
  function indicatorBias(data, which) {
    var n = data.length - 1;
    if (which === 'rsi') {
      var r = numVal(calcRSI(data, 14), n);
      return r == null ? null : (r >= 50 ? 'long' : 'short');
    }
    if (which === 'macd') {
      var h = numVal(calcMACD(data).histogram, n);
      return h == null ? null : (h >= 0 ? 'long' : 'short');
    }
    if (which === 'bb') {
      var bb = calcBollinger(data, 20, 2);
      var mid = numVal(bb.middle, n), c = data[n].close;
      if (mid == null) return null;
      return c >= mid ? 'long' : 'short';
    }
    if (which === 'ema') {
      var e = numVal(calcEMA(data, 21), n), c2 = data[n].close;
      if (e == null) return null;
      return c2 >= e ? 'long' : 'short';
    }
    if (which === 'sma') {
      var s = numVal(calcSMA(data, 50), n), c3 = data[n].close;
      if (s == null) return null;
      return c3 >= s ? 'long' : 'short';
    }
    return null;
  }

  // ===== REGIME ADAPTIF (slope regresi linier 100-bar, tanpa lookahead) =====
  // Klasifikasi kondisi market jadi NAIK / TURUN / RANGE dari kemiringan harga
  // normal (%/hari). Dipakai strategi 'adaptive' untuk memilih strategi terbaik
  // per regime, dan panel untuk menampilkan status adaptif.
  function adaptiveRegime(data, params) {
    params = params || {};
    var halfW = params.regimeWindow || 100;
    var thr = params.regimeThr || 0.03;
    var out = { regime: 'range', dir: null, label: 'RANGE (sideways)', slope: 0, ok: false };
    if (!data || data.length < 60) return out;
    // gunakan separuh window terakhir utk deteksi regime bersih (bukan 100 penuh di tengah)
    var start = data.length - halfW < 0 ? 0 : data.length - halfW;
    var segs = data.slice(start, data.length);
    if (segs.length < 30) return out;
    var n = segs.length, sx = 0, sy = 0;
    for (var i = 0; i < n; i++) { sx += i + 1; sy += segs[i].close; }
    var mx = sx / n, my = sy / n, sxy = 0, sxx = 0;
    for (var i = 0; i < n; i++) { sxy += (i + 1 - mx) * (segs[i].close - my); sxx += (i + 1 - mx) * (i + 1 - mx); }
    var slopePerDollar = sxx > 0 ? sxy / sxx : 0;
    var normSlope = slopePerDollar / (segs[n - 1].close || 1) * 100;
    out.slope = +normSlope.toFixed(4);
    out.ok = true;
    if (normSlope >= thr) { out.regime = 'trending'; out.dir = 'up'; out.label = 'TREND NAIK'; }
    else if (normSlope <= -thr) { out.regime = 'trending'; out.dir = 'down'; out.label = 'TREND TURUN'; }
    else { out.regime = 'ranging'; out.dir = null; out.label = 'RANGE (sideways)'; }
    return out;
  }
  // Klasifikasi kondisi pasar menjadi TRENDING (naik/turun) vs RANGING (sideways)
  // pakai kombinasi ADX + arah EMA + volatilitas relatif. Dipakai utk FILTER:
  // sinyal yang melawan regime saat ini berisiko lebih tinggi / di-mark WEAK.
  function detectRegime(data, params) {
    params = params || {};
    var out = { regime: 'ranging', dir: null, label: 'RANGE (sideways)', adx: 0, adxRaw: null, atrPct: 0, volRatio: 0, ok: false };
    if (!data || data.length < 40) return out;
    var atr = calcATR(data, 14);
    var atrArr = atr.filter(function (a) { return a !== null; });
    var lastATR = atrArr[atrArr.length - 1];
    var last = data[data.length - 1];
    var atrVal = lastATR ? lastATR.value : (last.high - last.low);
    var price = last.close || 1;
    out.atrPct = +((atrVal / price) * 100).toFixed(3);
    var past20 = data[Math.max(0, data.length - 21)];
    var past40 = data[Math.max(0, data.length - 41)];
    if (past40) out.volRatio = +(atrVal / Math.max(0.0001, (past40.high - past40.low) / 40 + (past20.high - past20.low) / 20)).toFixed(3);

    var adxSm = smoothADX(calcADX(data, 14), 14);
    var adx = adxSm.filter(function (a) { return a !== null; });
    var adxV = adx.length ? adx[adx.length - 1].value : null;
    if (adxV == null) return out;
    out.adxRaw = adxV;
    out.adx = +adxV.toFixed(1);

    var emaFast = calcEMA(data, 20);
    var emaSlow = calcEMA(data, 50);
    var ef = emaFast.filter(function (e) { return e !== null; }).slice(-1)[0];
    var es = emaSlow.filter(function (e) { return e !== null; }).slice(-1)[0];
    var dir = null;
    if (ef && es) {
      var diff = (ef.value - es.value) / Math.max(0.0001, es.value);
      if (Math.abs(diff) < 0.0015) dir = null; else dir = diff > 0 ? 'up' : 'down';
    }
    var adxThreshold = params.adx || 25;
    var trending = adxV >= adxThreshold && dir !== null;
    out.regime = trending ? 'trending' : 'ranging';
    out.dir = dir;
    out.ok = true;
    if (trending) out.label = dir === 'up' ? 'TREND NAIK' : 'TREND TURUN';
    else out.label = 'RANGE (sideways)';
    return out;
  }

  // ===== EDGE RANKER (adopsi "Structural SVM Ranker" ala LuxAlgo) =====
  // Skor 0-100 per sinyal dari komponen volume, momentum, dan ekspansi harga —
  // meniru skor kekuatan struktural. Komponen di-normalisasi & digabung bobot.
  function calcEdge(data, strategy, params, side) {
    params = params || {};
    side = side || 'long';
    var out = { score: 0, breakdown: {}, grade: 'NORMAL', vol: 0, mom: 0, exp: 0 };
    if (!data || data.length < 30) return out;
    var n = data.length - 1;
    var c = data[n];

    // volume component: volume rata2 14 terakhir vs baseline 60 — partisipasi/konfirmasi
    var volSum = 0, volN = 0, baseSum = 0, baseN = 0;
    for (var i = Math.max(1, n - 13); i <= n; i++) { volSum += data[i].volume || 0; volN++; }
    for (var j = Math.max(1, n - 59); j <= Math.max(1, n - 14); j++) { baseSum += data[j].volume || 0; baseN++; }
    var baseAvg = baseN ? baseSum / baseN : (volN ? volSum / volN : 1);
    var volRatio = baseAvg > 0 ? (volSum / Math.max(1, volN)) / baseAvg : 1;
    var volScore = Math.max(0, Math.min(1, (volRatio - 0.6) / 1.2)); /* <0.6x -> 0, >1.8x -> 1 */
    out.vol = +(+volScore * 100).toFixed(0);

    // momentum component: slope harga 10 bar relatif ATR (kekuatan dorongan)
    var p = c.close, p10 = data[Math.max(0, n - 10)].close;
    var atr = calcATR(data, 14).filter(function (a) { return a !== null; }).slice(-1)[0];
    var atrV = atr ? atr.value : (c.high - c.low) || 1;
    var slope = (p - p10) / Math.max(0.0001, atrV); /* dalam satuan ATR */
    var momScore = side === 'long' ? (slope + 1) / 2 : (-slope + 1) / 2; /* 0..1 */
    momScore = Math.max(0, Math.min(1, momScore));
    out.mom = +(+momScore * 100).toFixed(0);

    // expansion component: seberapa jauh jarak body relatif range 20 — ekspansi vs kontraksi
    var body = Math.abs(c.close - c.open);
    var hi20 = -Infinity, lo20 = Infinity;
    for (var k = Math.max(0, n - 19); k <= n; k++) { if (data[k].high > hi20) hi20 = data[k].high; if (data[k].low < lo20) lo20 = data[k].low; }
    var range20 = hi20 - lo20 || 1;
    var expansion = body / range20;
    var expScore = Math.max(0, Math.min(1, expansion * 6)); /* body 17% dari range -> 1 */
    out.exp = +(+expScore * 100).toFixed(0);

    // gabung bobot: momentum 0.45, volume 0.35, ekspansi 0.20
    var total = momScore * 0.45 + volScore * 0.35 + expScore * 0.20;
    out.score = Math.round(Math.max(0, Math.min(100, total * 100)));
    out.grade = out.score >= 70 ? 'STRONG' : out.score >= 45 ? 'NORMAL' : 'WEAK';
    out.breakdown = { vol: out.vol, mom: out.mom, exp: out.exp, volRatio: +volRatio.toFixed(2), slopeATR: +slope.toFixed(2), expansionPct: +(expansion * 100).toFixed(1) };
    return out;
  }

  function analyzeConfluence(data, strategy, params, higherTfData) {
    params = params || {};
    strategy = (strategy || 'rsi').toLowerCase();
    var res = { strategy: strategy, signal: null, score: 0, verdict: 'WEAK', reasons: [], aligned: 0, total: 0, gate: null, regime: null, edge: null };
    if (!data || data.length < 60) return res;

    // Bias utama dari strategi yg dipilih (sinyal crossing terakhir yg masih berlaku)
    var sigs = genSignals(data, strategy, params);
    var primary = 'flat';
    for (var i = sigs.length - 1; i >= 0; i--) { if (sigs[i] !== 'flat') { primary = sigs[i]; break; } }
    // Jika strategi tidak punya crossing (jarang), pakai bias indikator pembentuknya
    if (primary === 'flat') {
      var p = indicatorBias(data, strategy === 'ema' ? 'ema' : strategy === 'sma' || strategy === 'ma' ? 'sma' : strategy === 'adaptive' ? 'sma' : 'rsi');
      if (p) primary = p;
    }
    if (primary === 'flat' || primary === null) return res;
    res.signal = primary;
    res.total = 5;

    // Regime dari slope regresi (adaptif) — dipakai juga utk keputusan strategi adaptive
    res.slopeRegime = adaptiveRegime(data, params);
    ...
    checks.forEach(function (ch) {
      var b = indicatorBias(data, ch.st);
      if (b === null) return;
      var match = b === primary;
      if (match) aligned++;
      res.reasons.push(b === null ? '· ' + ch.label + ' belum tersedia'
        : (match ? '▲ ' + ch.label + ' searah BUY' : (b === 'long' ? '▲ ' : '▼ ') + ch.label + ' melawan — arah ' + b.toUpperCase()));
    });
    res.aligned = Math.min(aligned, res.total);
    res.score = Math.round((aligned / res.total) * 100);
    res.verdict = aligned >= 4 ? 'STRONG' : aligned >= 3 ? 'MODERATE' : 'WEAK';

    // Gerbang bias timeframe besar (D1): harga vs tren jangka panjang (SMA50)
    if (higherTfData && higherTfData.length > 60) {
      var trend = indicatorBias(higherTfData, 'sma');
      if (trend) {
        if (trend === primary) {
          res.gate = { status: 'ok', text: 'SEARAH trend D1' };
        } else {
          res.gate = { status: 'conflict', text: 'melawan trend D1' };
          res.reasons.push('⚠ melawan trend D1 — risikonya lebih tinggi');
          if (res.verdict === 'STRONG') res.verdict = 'MODERATE';
        }
      }
    }

    // REGIME FILTER (LuxAlgo style): apakah pasar trending atau ranging, & searah?
    var regime = detectRegime(data, params);
    res.regime = { regime: regime.regime, dir: regime.dir, label: regime.label, adx: regime.adx, atrPct: regime.atrPct };
    if (regime.regime === 'trending' && regime.dir) {
      var regSide = regime.dir === 'up' ? 'long' : 'short';
      res.regime.align = regSide === primary;
      if (!res.regime.align) {
        res.reasons.push('⚠ melawan regime: pasar ' + regime.label + ' (' + regime.dir.toUpperCase() + ') — sinyal berlawanan arah');
        if (res.verdict === 'STRONG') res.verdict = 'MODERATE';
      } else {
        res.reasons.push('✓ searah regime: pasar ' + regime.label);
      }
    } else {
      res.reasons.push('◻ regime: pasar RANGE — sinyal momentum di market sideways berisiko lebih tinggi');
    }

    // EDGE RANKER (LuxAlgo SVM style): kekuatan volume/momentum/ekspansi sinyal
    var edge = calcEdge(data, strategy, params, primary);
    res.edge = edge;
    if (edge.grade === 'WEAK' && res.verdict === 'STRONG') res.verdict = 'MODERATE';
    res.score = Math.round((res.score + edge.score) / 2);
    if (res.score < 30) res.verdict = 'WEAK';
    else if (res.score >= 65 && res.verdict !== 'STRONG' && res.aligned >= 3) res.verdict = 'STRONG';

    // Strategi ADAPTIVE: aktifkan routing regime (NAIK→all-long, TURUN→smc L/S, RANGE→bb-long)
    var decision = 'wait';
    var isAdaptive = strategy === 'adaptive';
    if (isAdaptive && res.slopeRegime && res.slopeRegime.ok) {
      var ar = res.slopeRegime;
      var routed = ar.regime === 'trending' ? (ar.dir === 'up' ? 'all (long)' : 'smc (long+short)') : 'bb (long)';
      res.reasons.push('⚙ ADAPTIF: regime ' + ar.label + ' → pakai sinyal ' + routed);
      // Di regime TURUN, izinkan SELL bila struktur pasar (smc) patah ke bawah
      // (sejalan dengan routing smc L/S). Di NAIK/RANGE tetap WAIT utk SELL.
      if (ar.dir === 'down' && res.signal === 'short' && res.verdict !== 'WEAK') {
        var _smaD = calcSMA(data, 100); var _nD = data.length;
        var _lastD = data[_nD - 1].close;
        var _sd = _smaD[_nD - 1], _sd2 = _smaD[_nD - 2];
        if (_sd && _sd2 && _lastD < _sd.value && _sd.value < _sd2.value) {
          decision = 'sell';
          res.reasons.push('✓ SELL diizinkan: regime TURUN terkonfirmasi + struktur patah (smc)');
        }
      }
    }

    // KEPUTUSAN FINAL BUY/SELL/WAIT — diverifikasi historis:
    //  * BUY  valid: hit-rate > baseline, ekspektasi positif (boleh di range/up,
    //    dilarang bila tren jangka panjang TURUN).
    //  * SELL tidak andal di market bull/ranging (hit-rate < 50%, return masih naik),
    //    jadi SELL HANYA dipicu bila ada KONFIRMASI tren turun (regime trending down
    //    ATAU harga di bawah SMA-100 yang menurun). Di range/bull → WAIT.
    //  * WAIT filter kualitas: return terendah → benar menyaring setup lemah.
    var longDown = null, longUp = null;
    if (data && data.length > 120) {
      var _sma = calcSMA(data, 100), _n = data.length;
      var _lastC = data[_n - 1].close;
      var _s = _sma[_n - 1], _s2 = _sma[_n - 2];
      if (_s && _s2) {
        longUp = _lastC > _s.value * 1.001 && _s.value > _s2.value;
        longDown = _lastC < _s.value * 0.999 && _s.value < _s2.value;
      }
    }
    var trending = res.regime && res.regime.regime === 'trending';
    var trendDown = res.regime && res.regime.dir === 'down';
    if (!isAdaptive) {
      if (res.signal && res.verdict === 'STRONG' && (!res.gate || res.gate.status === 'ok')) {
        if (res.signal === 'long') {
          // BUY dilarang saat kekuatan besar mulai turun (tren jangka panjang turun)
          if (!(trending && trendDown) && longDown !== true) decision = 'buy';
        } else if (res.signal === 'short') {
          // SELL HANYA di tren turun TERKONFIRMASI ganda: regime trending down
          // ATAU harga di bawah SMA-100 yang menurun. Di market yang bukan bear
          // berkelanjutan, SELL dianggap lemah (terbukti tidak valid) → WAIT.
          if (longDown === true) decision = 'sell';
        }
      }
    } else {
      // ADAPTIVE: BUY hanya boleh saat strategi aktif adalah all-long (NAIK) atau
      // bb-long (RANGE) — saring agar tidak buy lalu tren berbalik turun.
      if (res.signal && res.verdict !== 'WEAK' && (!res.gate || res.gate.status === 'ok')) {
        if (res.signal === 'long' && longDown !== true) decision = 'buy';
      }
    }
    res.longBias = longUp === true ? 'up' : (longDown === true ? 'down' : 'flat');
    res.decision = decision;
    return res;
  }

  // ===== STRUKTUR MULTI-TIMEFRAME DASHBOARD (adopsi "Structure & Trend Dashboard" ala LuxAlgo) =====
  // Bandingkan kesepakatan arah struktur (BOS/CHoCH + trend) antar pasangan timeframe
  // (mis. 1D/1H, 4H/M15). Tiap baris punya 'align': ✓ bila timeframe searah, ✗ bila divergen.
  // Sinyal terbaik umumnya saat semua timeframe searah (confluence).
  function mtfStructureDashboard(opts) {
    opts = opts || {};
    var pairs = opts.pairs || [['1D', '1H'], ['4H', 'M15']];
    var tfs = opts.tfs || {}; // { '1D': data, '1H': data, ... }
    var rows = [];
    var globalScore = 0, globalCount = 0;
    pairs.forEach(function (pair) {
      var hi = pair[0], lo = pair[1];
      var hiData = tfs[hi], loData = tfs[lo];
      var hiBias = hiData && hiData.length > 30 ? structureBias(hiData) : null;
      var loBias = loData && loData.length > 30 ? structureBias(loData) : null;
      if (!hiBias || !loBias) {
        rows.push({ pair: hi + '/' + lo, align: null, hiBias: hiBias || 'n/a', loBias: loBias || 'n/a', score: 0, label: 'data kurang' });
        return;
      }
      var agree = hiBias.side === loBias.side && hiBias.side !== 'flat';
      var score = agree ? 1 : -1;
      globalScore += score; globalCount++;
      rows.push({
        pair: hi + '/' + lo,
        align: agree,
        hiBias: hiBias.label,
        loBias: loBias.label,
        side: hiBias.side + '/' + loBias.side,
        score: score,
        label: agree ? 'SEARAH ✓' : 'DIVERGEN ✗'
      });
    });
    var confluence = globalCount ? (globalScore / globalCount) : 0;
    var verdict = confluence >= 0.5 ? 'SEARAH KUAT' : confluence <= -0.5 ? 'BERLAWANAN' : (confluence === 0 ? 'NETRAL' : 'CAMPURAN');
    return { rows: rows, score: globalScore, pairCount: globalCount, agreement: +confluence.toFixed(2), verdict: verdict };
  }

  function structureBias(data) {
    var ms = detectMarketStructure(data);
    var side = 'flat', text = ms.structure;
    if (/^uptrend/.test(ms.structure)) side = 'long';
    else if (/^downtrend/.test(ms.structure)) side = 'short';
    var last = data[data.length - 1];
    var ema = calcEMA(data, 21).filter(function (e) { return e !== null; }).slice(-1)[0];
    if (side === 'flat' && ema) side = last.close >= ema.value ? 'long' : 'short';
    return { side: side, label: text };
  }
  ...
  function analyze(data) {
    ...
    var adx = memoCalc('adx14', data, function () { return smoothADX(calcADX(data, 14), 14); });
    var aroon = memoCalc('aroon14', data, function () { return calcAroon(data, 14); });
    var cci = memoCalc('cci20', data, function () { return calcCCI(data, 20); });
    var wr = memoCalc('wr14', data, function () { return calcWilliamsR(data, 14); });
    ...
    var lastADX = adx.filter(function (a) { return a !== null; }).slice(-1)[0];
    var lastAroon = aroon[aroon.length - 1];
    var lastCCI = cci.filter(function (c) { return c !== null; }).slice(-1)[0];
    var lastWR = wr.filter(function (w) { return w !== null; }).slice(-1)[0];
    ...
    signals.push('ADX(14): ' + (lastADX ? lastADX.value.toFixed(1) : '-') + ' — tren ' + (lastADX ? (lastADX.value >= 25 ? 'KUAT' : lastADX.value >= 20 ? 'sedang' : 'lemah/tak ada tren') : 'n/a') + (lastADX && lastADX.value >= 25 ? ' (mengikuti tren)' : lastADX && lastADX.value >= 20 ? ' (menguat)' : ''));
    if (lastAroon && lastAroon.up !== null && lastAroon.down !== null) {
      signals.push('Aroon(14): Up ' + lastAroon.up.toFixed(0) + ' / Down ' + lastAroon.down.toFixed(0) + ' — ' + (lastAroon.up > lastAroon.down ? 'tren MENGUAT ke NA' : lastAroon.down > lastAroon.up ? 'tren MENGUAT ke TURUN' : 'sideways/imbang') + (lastAroon.up > 70 ? ' (up kuat)' : lastAroon.down > 70 ? ' (down kuat)' : ''));
    }
    if (lastCCI) {
      signals.push('CCI(20): ' + lastCCI.value.toFixed(1) + ' — ' + (lastCCI.value > 100 ? 'OVERBOUGHT' : lastCCI.value > 0 ? 'momentum naik' : lastCCI.value < -100 ? 'OVERSOLD' : 'momentum turun'));
    }
    if (lastWR) {
      signals.push('%R(14): ' + lastWR.value.toFixed(1) + ' — ' + (lastWR.value > -20 ? 'OVERBOUGHT' : lastWR.value < -80 ? 'OVERSOLD' : 'netral'));
    }
    if (bosEvents.length) {
    ...
    summary += '**RSI(14):** ' + (rsiVal || '-') + ' | **MACD:** ' + (lastMACD ? (lastMACD.value > 0 ? 'Bullish' : 'Bearish') : '-') + ' | **ADX(14):** ' + (lastADX ? lastADX.value.toFixed(1) : '-') + '\n';
    summary += '**Aroon Up/Dn:** ' + (lastAroon && lastAroon.up !== null ? lastAroon.up.toFixed(0) + '/' + lastAroon.down.toFixed(0) : '-') + ' | **CCI(20):** ' + (lastCCI ? lastCCI.value.toFixed(1) : '-') + ' | **%R(14):** ' + (lastWR ? lastWR.value.toFixed(1) : '-') + '\n';
  ...
  CC.ta = {
    ...
    formatMonteCarlo: formatMonteCarlo,
    walkforward: walkforward,
    formatWalkforward: formatWalkforward,
    analyzeConfluence: analyzeConfluence,
    detectRegime: detectRegime,
    calcEdge: calcEdge,
    mtfStructureDashboard: mtfStructureDashboard,
    ...
    addSignalAlert: addSignalAlert,
    removeSignalAlert: removeSignalAlert,
    clearSignalAlerts: clearSignalAlerts,
    listSignalAlerts: listSignalAlerts,
    checkSignalAlerts: checkSignalAlerts,
    formatSignalAlerts: formatSignalAlerts,
    marketOpen: marketOpen,
    marketStatusInfo: marketStatusInfo,
    barIsFresh: barIsFresh,
    panelDecision: panelDecision,
    sessionSpreadAt: sessionSpreadAt,
    listSignalLog: listSignalLog,
    clearSignalLog: clearSignalLog,
    formatSignalLog: formatSignalLog,
    analyze: analyze,
