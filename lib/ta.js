...
(function () {
  ...
  function fetchStooq(sym, iv) {
    ...
    function attempt() {
      ...
      return fetch(proxy + encodeURIComponent(url), { signal: AbortSignal.timeout(12000) })
        .then(function (r) { return r.ok ? r.text() : Promise.reject(new Error('Stooq HTTP ' + r.status)); })
        .then(function (txt) {
  ...
  function fetchNewsSentiment(symbol) {
    var sym = resolveSymbol(symbol);
    var friendly = sym === 'GC=F' ? 'gold' : sym === '^NDX' || sym === '^IXIC' ? 'nasdaq' : sym === '^VIX' ? 'vix' : sym.replace(/[^a-z]/gi, '').toLowerCase() || 'market';
    ...
    return fetch(ddgUrl, { signal: AbortSignal.timeout(12000) })
      ...
      .then(function (html) {
        if (!html || html.length < 200) { usedId = false; return ''; }
      ...
      .then(function () {
        ...
        sources.forEach(function (s) {
          var sc = scoreNewsSentiment(s);
          out.push({ text: s.slice(0, 140), score: sc });
        });
        var joined = sources.join(' ').slice(0, 3000);
        var agg = scoreNewsSentiment(joined);
        return { symbol: sym, friendly: friendly, headlines: out, aggregate: agg, sources: sources.length };
      })
  ...
  function clearAlerts() { _alerts = []; return { cleared: true }; }
