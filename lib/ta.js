...
(function () {
  ...
  function analyzeConfluence(data, strategy, params, higherTfData) {
    ...
    // eksekusi bila sinyal KUAT (STRONG), gerbang D1 tidak konflik,
    // dan tidak melawan regime ber-tren. Ketika pasar RANGE (tanpa tren)
    // tidak ada arah untuk 'melawan', jadi tidak memblokir.
    var decision = 'wait';
    if (res.signal && res.verdict === 'STRONG' && (!res.gate || res.gate.status === 'ok')) {
      var blocksRegime = res.regime && res.regime.regime === 'trending' && res.regime.align === false;
      if (!blocksRegime) {
        decision = res.signal === 'long' ? 'buy' : res.signal === 'short' ? 'sell' : 'wait';
      }
    }
