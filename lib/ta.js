...
(function () {
  ...
  /* Cek data terbaru terhadap daftar sinyal. Kembalikan array sinyal yang BARU
     memunculkan sinyal (fire hanya sekali per state per sinyal). Setiap sinyal
     membawa info sesi pasar (open/tutup) supaya tidak menyesatkan saat libur. */
  function checkSignalAlerts(data) {
    ...
    _signals = _signals.filter(function (s) {
      ...
      if (cur === 'long' || cur === 'short') {
        s.lastSignal = cur;
        if (prev !== cur && fresh) {
          /* crossing baru di bar terakhir barusan (hanya saat data masih segar) */
