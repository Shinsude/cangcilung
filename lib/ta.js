...
(function () {
    ...
    function attempt() {
      ...
      return fetch(proxy + encodeURIComponent(url), { signal: AbortSignal.timeout(12000) })
        ...
        .then(function (j) {
          ...
          var out = { symbol: sym, name: result.meta && result.meta.shortName || sym, interval: iv, data: data };
          setCache(ck, out);
          return out;
        })
        .catch(function (err) {
  ...
  function scoreSignals(signals, data) {
    ...
    signals.forEach(function (s) {
      if (s.indexOf('BULLISH') !== -1 || s.indexOf('bullish') !== -1 || s.indexOf('OVERSOLD') !== -1 || s.indexOf('naik') !== -1 || s.indexOf('rebound') !== -1) { score += 5; reasons.push(s.slice(0, 40)); }
      if (s.indexOf('BEARISH') !== -1 || s.indexOf('bearish') !== -1 || s.indexOf('OVERBOUGHT') !== -1 || s.indexOf('turun') !== -1 || s.indexOf('koreksi') !== -1) { score -= 5; reasons.push(s.slice(0, 40)); }
      if (s.indexOf('CONFLUENT') !== -1 || s.indexOf('confluent') !== -1) score += 8;
      if (s.indexOf('DIVERGENT') !== -1 || s.indexOf('divergent') !== -1) score -= 5;
    });
    score = Math.max(0, Math.min(100, score));
    var bias = score >= 65 ? 'BULLISH' : score <= 35 ? 'BEARISH' : 'NEUTRAL';
    var confidence = Math.abs(score - 50) * 2;
    return { score: score, bias: bias, confidence: confidence, label: bias + ' (' + confidence + '% confidence)' };
  }
