...
(function () {
  ...
  function runSim(data, strategy, params, exitFilter) {
    ...
    var cost = (params.cost || 0); /* biaya per-trade (spread+komisi) di mata uang quote, default 0 */
    ...
    for (var i = 1; i < data.length; i++) {
      if (pos === null && signals[i] !== 'flat') {
        ...
        if (closed) {
          if (cost > 0) pnl -= cost; /* kurangi biaya per trade */
          equity += pnl;
          var rec = { side: pos.side, pnl: +pnl.toFixed(2), exit: i, exitTime: data[i].time, entryTime: data[pos.i].time };
