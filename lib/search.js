...
(function () {
  ...
  var TICKER_HINTS = {
    'usa100': '^NDX', 'nasdaq100': '^NDX', 'ndx': '^NDX', 'nasdaq': '^IXIC', 'dow': '^DJI',
    'djia': '^DJI', 's&p': '^GSPC', 's&p500': '^GSPC', 'sp500': '^GSPC', 'gspc': '^GSPC',
    'nikkei': '^N225', 'gold': 'GC=F', 'emas': 'GC=F', 'silver': 'SI=F', 'oil': 'CL=F',
    'minyak': 'CL=F', 'btc': 'BTC-USD', 'bitcoin': 'BTC-USD', 'eth': 'ETH-USD', 'ethereum': 'ETH-USD',
    'usd': 'USD', 'eur': 'EURUSD=X', 'idr': 'USDIDR=X', 'dolar': 'USDIDR=X', 'rupiah': 'USDIDR=X'
  };
  var TICKER_HINT_RE = /(usa100|nasdaq100|nasdaq|dow|djia|s&p\s?500|s&p|gspc|sp500|nikkei|gold|emas|silver|oil|minyak|btc|bitcoin|eth|ethereum|dolar|rupiah|usd|eur)/i;
  ...
  function fetchQuote(ticker) {
    var sym = (ticker || '').replace(/^[$^]/, '');
    if (sym.indexOf('^') === 0) sym = encodeURIComponent('^' + sym.slice(1));
    var url = 'https://query1.finance.yahoo.com/v8/finance/chart/' + sym + '?interval=1d&range=5d&includePrePost=false';
    return fetch(url, { signal: AbortSignal.timeout(12000) })
      ...
      .then(function (j) {
        ...
        var prev = (meta.chartPreviousClose != null) ? meta.chartPreviousClose : (meta.previousClose != null ? meta.previousClose : null);
        ...
        var ts = r.timestamp || [];
        var lastDay = (typeof closes === 'object' && closes.length) ? closes[closes.length - 1] : null;
        ...
        var when = meta.regularMarketTime ? new Date(meta.regularMarketTime * 1000) : null;
