...
(function () {
  ...
  function trainMLP(trainX, trainY, opt) {
    ...
    var lr = opt.lr || 0.03, epochs = opt.epochs || 500, l2 = opt.l2 || 0.001;
    ...
    function xavier(nIn) { return (Math.random() * 2 - 1) * Math.sqrt(1 / nIn); }
    ...
    var W3 = new Array(h2).fill(0).map(function () { return xavier(h2); }), b3 = 0;
    for (var e = 0; e < epochs; e++) {
      for (var s = 0; s < n; s++) {
        // forward
        var x = trainX[s];
    ...
    function predictProb(xrow) {
      var a1 = new Array(h1), z1 = new Array(h1);
      for (var i = 0; i < h1; i++) { z1[i] = b1[i]; for (var j = 0; j < f; j++) z1[i] += W1[i][j] * xrow[j]; a1[i] = tanh(z1[i]); }
      var a2 = new Array(h2), z2 = new Array(h2);
      for (var i2 = 0; i2 < h2; i2++) { z2[i2] = b2[i2]; for (var j2 = 0; j2 < h1; j2++) z2[i2] += W2[i2][j2] * a1[j2]; a2[i2] = tanh(z2[i2]); }
      var z3 = b3; for (var j3 = 0; j3 < h2; j3++) z3 += W3[j3] * a2[j3];
      return sig(z3);
    }
    return { kind: 'Deep MLP (vanilla)', engine: 'vanilla', predictProb: predictProb };
  }
  ...
  function trainDirection(data, opt) {
    ...
    var wantTF = !opt || opt.engine !== 'vanilla';
    function runVanilla() {
      try {
        var model = trainMLP(ds.trainX, ds.trainY, opt);
        ...
        return Promise.resolve({ ok: true, engine: 'vanilla', kind: model.kind, model: model, train: tr, test: te, scaler: ds.scaler, names: ds.names, ds: ds, H: ds.H });
    ...
    return loadTF(6000).then(function (tfOk) {
      if (tfOk) {
        return trainTF(ds.trainX, ds.trainY, opt).then(function (model) {
          var tr = evalModel(ds.trainX.map(function (r) { return model.predictProb(r); }), ds.trainY);
          var te = evalModel(ds.testX.map(function (r) { return model.predictProb(r); }), ds.testY, ds.trainY.filter(function (y) { return y === 1; }).length / ds.trainY.length);
          return { ok: true, engine: 'tfjs', kind: model.kind, model: model, train: tr, test: te, scaler: ds.scaler, names: ds.names, ds: ds, H: ds.H };
        }).catch(function () { return runVanilla(); });
      }
      return runVanilla();
    });
  }
  ...
  function scoreSignal(data, strategy, params, opt) {
    ...
    return predictDirection(data, opt).then(function (m) {
      if (m.error) return m;
