...
(function () {
  ...
  // Gabungkan 3 sumber menjadi FUSION bobot:
  //   wMl  : keyakinan ARAH dari Model A (pUp utk long, pDown utk short)
  //   wTa  : skor KONFLUENSI TA (5 indikator + regime + edge, 0..1) dari ta.analyzeConfluence
  //   wReg : kesesuaian sinyal terhadap REGIME pasar (trend/range) utk BUY vs SELL
  // Verdict akhir ditentukan oleh skor fusion (bukan hanya ML), lebih konservatif.
  ...
  function scoreSignal(data, strategy, params, opt) {
    ...
    var build = function (m) {
      ...
      var alignProb;
      if (primary === 'long') alignProb = m.pUp;
      else if (primary === 'short') alignProb = m.pDown;
      else alignProb = Math.max(m.pUp, m.pDown);

      // Konfluensi TA (bisa dimatikan via opt.skipConfluence utk tes/speed)
      var conf = null;
      if (!opt.skipConfluence) {
        try {
          var c = ta.analyzeConfluence ? ta.analyzeConfluence(data, strategy, params) : null;
          if (c && typeof c.score === 'number') {
            conf = { score: c.score, verdict: c.verdict, decision: c.decision || 'wait', regime: c.regime ? c.regime.regime : null, regimeDir: c.regime ? c.regime.dir : null, regimeLabel: c.regime ? c.regime.label : null };
          }
        } catch (e) { conf = null; }
      }
      var taP = conf ? conf.score / 100 : 0.5; // 0..1 (0.5 = netral bila tak ada)

      // Kesesuaian regime: long di uptrend/range nyaman; long di downtrend berisiko.
      var regimeP = 0.5;
      if (conf) {
        if (conf.regimeDir === 'up' && primary === 'long') regimeP = 1;
        else if (conf.regimeDir === 'down' && primary === 'short') regimeP = 1;
        else if (conf.regimeDir === 'up' && primary === 'short') regimeP = 0.15;
        else if (conf.regimeDir === 'down' && primary === 'long') regimeP = 0.2;
        else if (conf.regime === 'ranging') regimeP = (primary === 'long') ? 0.75 : 0.35; // SELL lemah di range
      }

      var w = opt.weights || { ml: 0.55, ta: 0.35, regime: 0.10 };
      var fusion = +(Math.max(0, Math.min(1, w.ml * alignProb + w.ta * taP + w.regime * regimeP))).toFixed(3);

      var verdict;
      if (primary === 'long') verdict = fusion >= 0.66 ? 'BUY KUAT' : fusion >= 0.55 ? 'BUY diizinkan' : 'BUY lemah';
      else if (primary === 'short') verdict = fusion >= 0.66 ? 'SELL KUAT' : fusion >= 0.55 ? 'SELL diizinkan' : 'SELL lemah';
      else verdict = 'WAIT (tak ada sinyal)';
      if (conf && conf.decision === 'wait' && primary !== 'flat' && verdict.indexOf('KUAT') === 0) {
        verdict = primary === 'long' ? 'BUY lemah (TA belum bulat)' : 'SELL lemah (TA belum bulat)';
      }

      return {
        ok: true, engine: m.engine, strategy: strategy,
        signal: primary, pDirectionUp: m.pUp, pDirectionDown: m.pDown,
        pSignalCorrect: +(alignProb).toFixed(3), confidence: m.confidence, H: m.H,
        fusion: fusion, fusionPct: Math.round(fusion * 100),
        weights: w, taScore: taP, regimeP: regimeP,
        confluence: conf,
        verdict: verdict
      };
    };
  ...
  function formatSignalScore(s, symbol) {
    ...
    var out = '## 🎯 Skor Sinyal Fusion — ' + (symbol || 'XAUUSD').toUpperCase() + ' (' + s.strategy.toUpperCase() + ')\n\n';
    ...
    out += '**Fusion ML×TA: ' + s.fusionPct + '/100**' + (s.fusion != null ? ' (_ML ' + s.weights.ml + '·' + Math.round((s.pSignalCorrect || 0) * 100) + '% + TA ' + s.weights.ta + '·' + Math.round((s.taScore || 0) * 100) + '% + regime ' + s.weights.regime + '·' + Math.round((s.regimeP || 0) * 100) + '%)' : '') + '\n';
    if (s.confluence) out += 'Konfluensi TA: **' + s.confluence.score + '/100** (' + s.confluence.verdict + ') · regime: ' + (s.confluence.regimeLabel || '-') + '\n';
    out += 'Keputusan: **' + s.verdict + '**\n';
