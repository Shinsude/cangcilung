...
(function () {
  ...
  function saveModelCache(k, state) {
    try { if (typeof localStorage !== 'undefined') localStorage.setItem(k, JSON.stringify(state)); } catch (e) {}
  }
  function loadModelCache(k) {
    try { if (typeof localStorage !== 'undefined') { var v = localStorage.getItem(k); return v ? JSON.parse(v) : null; } } catch (e) {}
    return null;
  }
  ...
  CC.ml = {
    ...
    mlpState: mlpState,
    restoreMLP: function (st) { return st ? freshMLPFromState({ f: st.f || 16 }, st) : null; }
  };
})();
